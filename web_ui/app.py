...
def admin_dashboard():
    ...
    try:
        ...
        bookings_response = requests.get(f"{BOOKING_SERVICE_URL}/bookings?limit=5")
        destinations_response = requests.get(f"{DESTINATION_SERVICE_URL}/destinations")
        
        users = users_response.json()['users'] if users_response.status_code == 200 else []
        bookings = bookings_response.json()['bookings'] if bookings_response.status_code == 200 else []
        destinations = destinations_response.json()['destinations'] if destinations_response.status_code == 200 else []
        
        # Urutkan destinasi berdasarkan ID secara descending dan ambil 5 terbaru
        ...
        stats = {
            ...
            'recent_destinations': recent_destinations  # Tambahkan ini
        }
    except requests.RequestException:
        stats = {
            'users_count': 0,
            ...
            'recent_bookings': [],
            'recent_destinations': []  # Tambahkan ini untuk error case
        }
